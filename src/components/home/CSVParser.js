...
export function CSVConcat(CSVarray) {
    let newArr = [];

    for (let i = 0; i < CSVarray.length; i++) {
        let stringBuilder = "";

        if (i - 1 >= 0 && CSVarray[i].length != CSVarray[i - 1].length) {
            continue;
        }

        for (let j = 0; j < CSVarray[i].length - 1; j++) {
            stringBuilder = stringBuilder + '"' + CSVarray[i][j] + '"' + ",";
        }
        stringBuilder =
            stringBuilder + '"' + CSVarray[i][CSVarray[i].length - 1] + '"';

        newArr.push(stringBuilder);
    }
    return newArr;
}
